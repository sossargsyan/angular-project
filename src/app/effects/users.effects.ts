...
export class UsersEffects {
    ...
    switchMap((state: any) => {
      return this.userService.addUser(state.payload).pipe(
        ...
        map((response: any) => {
          ...
          state.dialogRef.close(response);
          return {
  ...
  @Effect()
  updateUserInServer = this.actions$.pipe(
    ofType(UsersActions.UPDATE_USER_IN_SERVER),
    switchMap((state: any) => {
      console.log(state.payload);
      return this.userService.updateUser(state.payload)
        .pipe(
          take(1),
          map(response => {
            state.dialogRef.close(response);
            return {
              type: UsersActions.UPDATE_USER_IN_STORE,
              payload: state.payload
            }
          }),
          catchError(errorCase => {
